...
export const CommandForm = ({ id, handleCloseModal }) => {
    ...
    const onSubmit = (form) => {
        const _form = { ...form };
        if (isCustom) _form.customServices = transformForm(form.customServices);
        if (!id) postData(_form);
        else {
    ...
    const PropertyButton = ({ data, value, control }) => {
                        ...
                        <div>
                            ...
                            {fields.map((item, index) => {
                                return (
                                    <div
                                        key={item.id}
                                        className="mb-3 rounded p-3 pr-10 bg-black/10 gap-1 relative overflow-y-auto"
                                    >
