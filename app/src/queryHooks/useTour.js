...
const fetchPaginatedDatas = (page, sortValue, sortDirection, searchValue, filters) => {
    ...
    if (filters.scheduledAt !== "") options += "&scheduledAt[after]=" + filters.scheduledAt +"&scheduledAt[before]=" + dayjs(filters.scheduledAt).add(1, 'day').format('YYYY-MM-DD')
...
const fetchDate = ({ queryKey }) => {
    ...
    return request({ url: API + "?scheduledAt[strictly_before]=" + dayjs(date).add(1, 'day').format('YYYY-MM-DD') + "&scheduledAt[strictly_after]=" + dayjs(date).subtract(1, 'day').format('YYYY-MM-DD') + '&user=' + user, method: 'get' })
}
...
const putData = form => {
    const _form = { ...form }
    if (form.scheduledAt) _form.scheduledAt = dayjs.utc(form.scheduledAt).local().format()
