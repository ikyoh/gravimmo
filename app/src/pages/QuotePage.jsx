...
import { useGetID, usePostData, usePutData } from "queryHooks/useQuote";
...
export const QuotePage = ({ title }) => {
    ...
    useEffect(() => {
        if (isSuccess) setQuoteData(data);
    }, [isSuccess]);
    ...
    const downloadFile = (id, chrono) => {
        ...
        axios({
            ...
            fileDownload(response.data, "devis" + chrono + ".pdf");
        });
    };

    const handleDeleteService = (index) => {
        let _content = quoteData.content
            .slice(0, index)
            .concat(quoteData.content.slice(index + 1));
    ...
    const handleAddService = (service) => {
        ...
        const _content = [...quoteData.content, _service];
        handleCloseModal();
        const { amountHT, amountTTC } = calcAmounts(_content);
        setQuoteData({
            ...quoteData,
    ...
    if (isLoading) return <Loader />;
        ...
        return (
            <>
                ...
                >
                    {!_.isEqual(data.content, quoteData.content) && (
                        <Button
                            size={ButtonSize.Big}
                    ...
                    <Button
                        size={ButtonSize.Big}
                        onClick={() =>
                            navigate("/quotes", { state: previousPageState })
                        }
                    >
                        <MdArrowBack />
                    </Button>
                    ...
                    <div className="flex flex-col gap-3 w-64">
                        ...
                        <div className="dark:bg-gradient-page rounded p-3">
                            <div className="flex justify-between">
                                <p>Total HT</p>
                                <p>{price(quoteData.amountHT)}</p>
                            </div>
                            <div className="flex justify-between">
                                <p>TVA ({quoteData.tva}%)</p>
                                <p>
                                    {price(
                                        quoteData.amountTTC - quoteData.amountHT
                                    )}
                                </p>
                            </div>
                            <div className="flex justify-between">
                                <p>Total TTC</p>
                                <p>{price(quoteData.amountTTC)}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <Table>
                    ...
                    <Tbody>
                        {quoteData.content?.map((content, index) => (
                            <Tr key={uuid()}>
