...
import { useGetPaginatedDatas, usePutData } from 'queryHooks/useQuote'
...
import { MdOutlineFileDownload, MdOutlineAssignment } from 'react-icons/md'
...
export const QuotesPage = ({ title }) => {
	...
	const { sortValue, sortDirection, handleSort } = useSortBy(initialPageState ? { value: initialPageState.sortValue, direction: initialPageState.sortDirection } : { value: "chrono", direction: "DESC" })
	...
	const calcTotalHTTVA10 = () => {
		return price(checkedList.reduce((acc, curr) => { if (curr.tva === 10) { return (acc + curr.amountHT) } return acc }
			, 0))
	}

	const calcTotalHTTVA20 = () => {
		return price(checkedList.reduce((acc, curr) => { if (curr.tva === 20) { return (acc + curr.amountHT) } return acc }
			, 0))
	}

	const calcTotalHT = () => {
		return price(checkedList.reduce((acc, curr) =>
			acc + curr.amountHT
			, 0))
	}
		...
		<>
			...
			>
						...
						<div className="chat chat-end">
							<div className="chat-bubble !max-w-none">
								<div className='mt-0.5 flex flex-row gap-3'>
									<p>
										TVA(10%) : {calcTotalHTTVA10()}
									</p>
									<p>
										TVA(20%) : {calcTotalHTTVA20()}
									</p>
								</div>
							</div>
						</div>
				...
				<Dropdown type='button' isDisabled={checkedList.length === 0}>
					...
					<button
						onClick={() => navigate("/commands/" + data.id, { state: { page: page, sortDirection: sortDirection, sortValue: sortValue, searchValue: searchValue } })}
					>
				...
				<Button
					size={ButtonSize.Big}
					onClick={() => handleOpenModal({ title: "Nouvelle facture", content: <InvoiceForm handleCloseModal={handleCloseModal} /> })}
				/>
			</Header>
				...
				: <Table>
					...
					<Tbody>
						{!isLoading && data['hydra:member'].map(data =>
							<Tr key={data.id} onClick={() => navigate("/invoices/" + data.id, { state: { page: page, sortDirection: sortDirection, sortValue: sortValue, searchValue: searchValue } })}>
								<Td text={data.chrono} />
								<Td>
									<div>
										<input
								...
								<Td label="Commande" text={data.command && data.command.id ? data.command.id : "..."} />
								<Td label="Syndic" text={data.trusteeTitle || data.customerTitle || "..."} />
								<Td label="Copropriété" text={data.propertyTitle ? data.propertyTitle : "..."} />
								<Td label="Date" text={dayjs(data.createdAt).format('DD/MM/YYYY')} />
								<Td label="Montant" text={data.formattedAmountHT} />
								<Td label="Statut">
									<div className="flex flex-row justify-between w-full">
										<div className='flex items-center'>
											<Dot color={statusColor[data.status]} />
											<span className='ml-1 mr-3 capitalize'>{data.status}</span>
										</div>
										<div className='flex items-center'>
											{(data.refundReference || data.isRefund) && <IoReloadCircleOutline size={23} />}
											{data.refundReference && "F-" + data.refundReference}
											{data.isSend && <IoIosSend size={23} />}
										</div>
									</div>
								</Td>
								<Td label="" text={""}>
									<Dropdown type='table'>
										...
										{data.command &&
											<button
												onClick={() => handleOpenModal({ title: "Commande", content: <CommandPage isModalContent={true} commandIRI={data.command["@id"]} /> })}>
