    ...
    return {
        ...
        filter:
            <Dropdown type='button' icon={<LuSettings2 size={26} />}>
                ...
                <div className="form-control">
                    <label className="label cursor-pointer m-0 p-0">
                        <span className="label-text">Lettré</span>
                        <input
                            type="radio"
                            name="status"
                            value="lettré"
                            className={`radio checked:bg-${statusColor["lettré"]}`}
                            checked={filters.status === "lettré"}
                            onChange={handleChangeInput}
                        />
                    </label>
                </div>
                <div className="form-control">
                    <label className="label cursor-pointer m-0 p-0">
                        <span className="label-text">Irrécouvrable</span>
                        <input
                            type="radio"
                            name="status"
                            value="irrécouvrable"
                            className={`radio checked:bg-${statusColor["irrécouvrable"]}`}
                            checked={filters.status === "irrécouvrable"}
                            onChange={handleChangeInput}
                        />
                    </label>
                </div>
                <div className="form-control">
                    <label className="label cursor-pointer m-0 p-0">
                        <span className="label-text">Avoir</span>
                        <input
                            type="checkbox"
                            name="refundReference"
                            className="toggle toggle-info"
                            checked={filters.refundReference}
                ...
                <div className="form-control mb-3 h-6">
                    <label className="label cursor-pointer m-0 p-0">
                        <span className="label-text">
                            {_.isEqual(filters, defaultFilters) ? "Aucun filtre" : "Réinitialiser"}
                        </span>
                        {!_.isEqual(filters, defaultFilters) &&
                            <button className="bg-secondary rounded-full h-6 w-6 flex items-center justify-center" style={{}}
